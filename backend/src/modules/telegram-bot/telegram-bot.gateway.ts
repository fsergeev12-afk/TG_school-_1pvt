...
export class TelegramBotGateway implements OnModuleInit {
  ...
  private setupCommandHandlers() {
    ...
    bot.onText(/\/start(.*)/, async (msg, match) => {
      ...
      if (startParam) {
        // Если есть параметр - это активация ученика
        this.logger.log(`Параметр start: ${startParam}`);
        
        // Здесь будет логика активации (создадим в модуле auth)
        await this.telegramBotService.sendMessage(
          chatId,
          '✅ Активация успешна! Переходи в Mini App, чтобы начать обучение.',
        );
      } else {
